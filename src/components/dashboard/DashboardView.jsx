...
export const DashboardView = ({ 
  ...
  const [viewMode, setViewMode] = useState(isUserAdmin ? 'cards' : 'table'); // 'cards' | 'table'
  ...
  // Group links by owner email for interactive card view
  const ownersWithLinks = useMemo(() => {
    const map = new Map();
    links.forEach(l => {
      ...
      if (!map.has(email)) {
        map.set(email, {
          ...
          links: [],
          totalClicks: 0,
          latestCreatedAt: l.createdAt
        });
      }
      const entry = map.get(email);
      entry.links.push(l);
      entry.totalClicks += (l.clicks || 0);
      if (new Date(l.createdAt) > new Date(entry.latestCreatedAt)) {
        entry.latestCreatedAt = l.createdAt;
      }
    });

    const list = Array.from(map.values()).sort((a, b) => b.totalClicks - a.totalClicks);
    return list;
  }, [links]);

  // Filtered owners based on search query (for cards mode)
  const filteredOwners = useMemo(() => {
    const query = searchQuery.toLowerCase().trim();
    if (!query) return ownersWithLinks;
    return ownersWithLinks.filter(owner => {
      const matchesOwnerInfo = owner.email.includes(query) || owner.name.toLowerCase().includes(query);
      const matchesAnyLink = owner.links.some(l => 
        (l.name || '').toLowerCase().includes(query) ||
        (l.shortCode || '').toLowerCase().includes(query) ||
        (l.targetUrl || '').toLowerCase().includes(query)
      );
      return matchesOwnerInfo || matchesAnyLink;
    });
  }, [ownersWithLinks, searchQuery]);

  // Filtered links (for flat table mode)
      ...
      <div className="neu-panel" style={{ padding: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem', gap: '0.75rem', flexWrap: 'wrap' }}>
          <div style={{ position: 'relative', flex: 1, minWidth: '240px' }}>
            ...
            <input 
              type="text"
              placeholder={isUserAdmin ? "Cari email pemilik, nama member, short code, atau URL..." : "Cari nama, short code, atau URL tujuan..."}
          ...
          {isUserAdmin && (
            <div className="neu-panel-inset" style={{ display: 'inline-flex', padding: '0.25rem', gap: '0.25rem', borderRadius: '10px' }}>
              <button
                type="button"
                onClick={() => setViewMode('cards')}
                className={`btn btn-sm ${viewMode === 'cards' ? 'btn-primary' : 'btn-ghost'}`}
                style={{ padding: '0.45rem 0.75rem', fontSize: '0.775rem', fontWeight: 800 }}
                title="Tampilkan Kartu Akun Member"
              >
                <Users size={14} />
                <span>Kartu per Email ({ownersWithLinks.length})</span>
              </button>
              <button
                type="button"
                ...
                style={{ padding: '0.45rem 0.75rem', fontSize: '0.775rem', fontWeight: 800 }}
                title="Tampilkan Tabel Semua Tautan"
              >
                <List size={14} />
                <span>Semua Tautan ({links.length})</span>
              </button>
            </div>
          )}
            ...
            {filteredOwners.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '3.5rem 1rem' }}>
                ...
                <h3 style={{ marginBottom: '0.35rem', fontWeight: 800, fontSize: '1.1rem' }}>
                  {searchQuery ? 'Akun Tidak Ditemukan' : 'Belum Ada Akun Member'}
                </h3>
                <p style={{ color: 'var(--text-muted)', maxWidth: '400px', margin: '0 auto', fontSize: '0.85rem', fontWeight: 600 }}>
                  {searchQuery ? 'Tidak ada email member yang sesuai dengan pencarian.' : 'Tautan yang dibuat oleh member akan dikelompokkan berdasarkan kartu email masing-masing.'}
                </p>
              </div>
            ) : (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.25rem' }}>
                {filteredOwners.map(owner => {
                  const isExpanded = expandedEmail === owner.email;
                  ...
                  return (
                    <div 
                      ...
                      style={{ 
                        padding: '1.25rem',
                        borderRadius: '16px',
                        border: isExpanded ? '2px solid var(--primary)' : '1px solid var(--border-subtle)',
                    ...
                    >
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.9rem' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', overflow: 'hidden' }}>
                            <img 
                              src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(owner.email)}`}
                              alt={owner.name}
                              style={{ width: '42px', height: '42px', borderRadius: '50%', border: '1px solid var(--border-subtle)', background: 'var(--bg-main)', flexShrink: 0 }}
                            />
                            <div style={{ overflow: 'hidden' }}>
                              <div style={{ fontWeight: 900, fontSize: '0.95rem', color: 'var(--text-main)', lineHeight: 1.2, textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}>
                                {owner.name}
                              </div>
                              <div style={{ fontSize: '0.75rem', fontFamily: 'var(--font-mono)', color: 'var(--primary)', fontWeight: 700, textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }} title={owner.email}>
                                {owner.email}
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="neu-panel-inset" style={{ display: 'flex', justifyContent: 'space-around', padding: '0.75rem 0.5rem', borderRadius: '10px', marginBottom: '0.85rem' }}>
                          <div style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: '1.25rem', fontWeight: 900, color: 'var(--primary)', lineHeight: 1 }}>{owner.links.length}</div>
                            <div style={{ fontSize: '0.65rem', fontWeight: 800, color: 'var(--text-muted)', marginTop: '0.25rem', letterSpacing: '0.04em' }}>TOTAL TAUTAN</div>
                          </div>
                          <div style={{ width: '1px', background: 'var(--border-subtle)' }} />
                          <div style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: '1.25rem', fontWeight: 900, color: 'var(--accent-emerald)', lineHeight: 1 }}>{owner.totalClicks}</div>
                            <div style={{ fontSize: '0.65rem', fontWeight: 800, color: 'var(--text-muted)', marginTop: '0.25rem', letterSpacing: '0.04em' }}>TOTAL KLIK</div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        <button
                          type="button"
                          onClick={() => toggleExpandEmail(owner.email)}
                          className={`btn ${isExpanded ? 'btn-primary' : 'btn-ghost'}`}
                          style={{ width: '100%', padding: '0.65rem', fontSize: '0.825rem', fontWeight: 800, justifyContent: 'center', gap: '0.4rem', borderRadius: '10px' }}
                        >
                          <LinkIcon size={14} />
                          <span>{isExpanded ? 'Tutup Daftar Tautan' : `Buka ${owner.links.length} Tautan Milik Akun Ini`}</span>
                          {isExpanded ? <ChevronUp size={15} /> : <ChevronDown size={15} />}
                        </button>

                        {isExpanded && (
                          <div style={{ marginTop: '1rem', paddingTop: '0.85rem', borderTop: '2px dashed var(--border-subtle)', display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
                            {owner.links.map(link => {
                              ...
                              return (
                                <div 
                                  ...
                                  style={{ padding: '0.85rem', borderRadius: '12px', background: 'var(--bg-main)' }}
                                >
                                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.35rem', gap: '0.5rem', flexWrap: 'wrap' }}>
                                    ...
                                    <span className="badge badge-primary" style={{ fontSize: '0.7rem', padding: '0.15rem 0.5rem', fontWeight: 800 }}>
                                      {link.clicks || 0} klik
                                    </span>
                                  </div>

                                  <div style={{ fontSize: '0.825rem', fontWeight: 800, color: 'var(--text-main)', marginBottom: '0.2rem' }}>
                                    {link.name || 'Tanpa Nama'}
                                    {link.mode === 'video' && <span className="badge badge-amber" style={{ fontSize: '0.625rem', padding: '0.1rem 0.35rem', marginLeft: '0.35rem' }}>Video</span>}
                                  </div>
                                  <a 
                                    href={link.targetUrl} 
                                    target="_blank" 
                                    rel="noreferrer"
                                    style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.25rem', wordBreak: 'break-all', marginBottom: '0.65rem' }}
                                  >
                                    <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '85%' }}>{link.targetUrl}</span>
                                    <ExternalLink size={11} style={{ flexShrink: 0 }} />
                                  </a>

                                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(60px, 1fr))', gap: '0.35rem' }}>
                                    <button onClick={() => handleCopy(fullShortUrl, link.id)} className="btn btn-sm" style={{ padding: '0.35rem 0.4rem', fontSize: '0.725rem', justifyContent: 'center' }} title="Salin Tautan">
                                      {copiedId === link.id ? <Check size={12} style={{ color: 'var(--accent-emerald)' }} /> : <Copy size={12} />}
                                      <span>Salin</span>
                                    </button>
                                    <button onClick={() => { setQrModalUrl(fullShortUrl); setQrModalOpen(true); }} className="btn btn-sm" style={{ padding: '0.35rem 0.4rem', fontSize: '0.725rem', justifyContent: 'center' }} title="QR Code">
                                      <QrCode size={12} style={{ color: 'var(--primary)' }} />
                                      <span>QR</span>
                                    </button>
                                    <button onClick={() => onSelectLinkAnalytics(link.id)} className="btn btn-sm" style={{ padding: '0.35rem 0.4rem', fontSize: '0.725rem', justifyContent: 'center' }} title="Analitik Detail">
                                      <BarChart2 size={12} />
                                      <span>Stat</span>
                                    </button>
                                    <button onClick={() => onEditLink(link)} className="btn btn-sm" style={{ padding: '0.35rem 0.4rem', fontSize: '0.725rem', justifyContent: 'center' }} title="Edit Tautan">
                                      <Edit3 size={12} />
                                    </button>
                                    <button onClick={() => onDeleteLink(link.id)} className="btn btn-danger btn-sm" style={{ padding: '0.35rem 0.4rem', fontSize: '0.725rem', justifyContent: 'center' }} title="Hapus Tautan">
                                      <Trash2 size={12} />
                                    </button>
                                  </div>
                                </div>
