...
const FEATURE_COLORS = ['#e11d48', '#f43f5e', '#10b981', '#8b5cf6', '#f59e0b', '#ec4899', '#06b6d4', '#6366f1', '#10b981'];
...
export const LandingView = ({ onNavigate, links = [], analyticsLogs = [], domains = [], currentUser = null, onOpenGoogleLogin, onLogout, onSaveLink }) => {
  ...
  const stats = useMemo(() => {
    ...
    const clicksFromLinks = links.reduce((sum, l) => sum + (l.clicks || 0), 0);
    const clicksFromLogs = (analyticsLogs || []).length;
    const totalClicks = Math.max(clicksFromLinks, clicksFromLogs);
    const cutoff = Date.now() - 24 * 60 * 60 * 1000;
    const recentIps = new Set(
      (analyticsLogs || [])
        .filter(l => l.timestamp && new Date(l.timestamp).getTime() > cutoff)
        .map(l => l.ip)
    );
    const activeVisitors = Math.max(recentIps.size, totalClicks > 0 ? 1 : 0);
    ...
    return [
      ...
      { label: t.statActive, value: fmt(activeVisitors) },
    ];
  }, [links, analyticsLogs, lang, t]);
